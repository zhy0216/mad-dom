...
let table = "# All task-02 source-comparison rows\n\nA = frozen reference; B = candidate, same runtime and FFI mode. All initial and supplemental samples are retained. Positive change is slower. Core/Testing operations use the median of per-round sums. Hotspots include 32 complete operations and all original materialization. Group changes are not confidence intervals.\n";
