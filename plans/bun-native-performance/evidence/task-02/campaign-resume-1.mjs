...
const name = `resume-1-${lane}-source-${ffi}-${kind}`;
...
const capture = () => ({ candidateSource: sourceManifest(root), candidateImage: fileIdentity(join(root, "build/mad-dom.node")),
  ...
  driver: Object.fromEntries(["campaign.mjs", "command.mjs", "smokes.mjs", "supplement.mjs", "method.md", "campaign-resume-1.mjs", "supplement-resume-1.mjs", "resume-1.mjs", "resume-1-method.md", "recovery-20260908/audit.json"].map(p => [p, digest(readFileSync(join(evidence, p)))])) });
...
if (kind === "formal") {
  ...
  if (index > 0) {
    const previous = JSON.parse(readFileSync(join(evidence, `resume-1-${order[index - 1]}-formal/manifest.json`), "utf8"));
...
if (kind === "supplemental") {
  const initial = JSON.parse(readFileSync(join(evidence, `resume-1-${lane}-source-${ffi}-formal/summary.json`), "utf8"));
