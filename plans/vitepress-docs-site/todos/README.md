...
| `02-pages-workflow.md` | P0 | easy | ✅ 已完成 · `docs.yml` 发布 workflow + `gh` 开启 Pages（workflow 源）+ 确认首次部署 |

## 文件（执行顺序）

1. ~~`01-vitepress-scaffold.md`~~ ✅ 已完成（归档于 `done/`）— 依赖：无
2. ~~`02-pages-workflow.md`~~ ✅ 已完成（归档于 `done/`）— 依赖 01（需要 `docs:build` script 与站点源）
