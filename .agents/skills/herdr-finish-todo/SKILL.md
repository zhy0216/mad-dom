...
description: 使用 Herdr 为 todo 队列创建独立 Git worktree，并在最多 5 个并行任务中启动 auto 模式的 OpenCode（DeepSeek V4 Flash）；每个任务完成后先 rebase 原分支并解决冲突，再合并、清理 worktree、补充下一项。仅在用户显式调用 $herdr-finish-todo、输入 /herdr-finish-todo，或明确要求用 Herdr 并行清完 todos 时使用。
---
...
3. 确认 `opencode` 可执行，并确认模型存在：
   ...
   必须使用 `deepseek/deepseek-v4-flash`。即使它当前是默认模型，也要显式传入；不可静默改用其他模型。
...
对仍处于 `working`/Thinking 的 OpenCode 采用 **5 分钟监控节奏**：完成一次批量状态检查后，若没有 `done`、`idle` 或 `blocked` 等可操作状态，选择一个正在工作的 agent 做最长 5 分钟的状态感知等待：

```bash
herdr agent wait <agent-name> --timeout 300000
```

wait 若因 agent settle 而提前返回，立即处理并同时刷新所有任务；若返回 `timeout`，这只是下一个 5 分钟检查点，不是失败、停滞或需要中断的证据。在该检查点用一次 `herdr agent list` 批量刷新，只对状态变化、完成或 blocked 的 agent 进一步 `get/read`，然后再进入下一个 5 分钟等待。不要用 15 秒或其他短超时循环高频轮询。

OpenCode 在复杂任务上可能长时间保持 Thinking。不得仅因 Thinking 持续时间长、一个或多个 5 分钟周期内状态未变、画面相同或 worktree 暂无改动，就发送 `esc`/`ctrl+c`、重启 agent 或改写任务。只有出现明确的错误、进程退出、需要交互的 `blocked` 状态，或用户明确要求停止时，才停止正常等待；若怀疑真正死循环，先收集 `get/read` 和 worktree 证据并向用户报告，不要擅自中断。
...
- OpenCode 必须通过 Herdr 启动，并显式使用 `--auto --model deepseek/deepseek-v4-flash`。
- OpenCode 处于 `working`/Thinking 时默认每 5 分钟批量检查一次；短超时或长时间 Thinking 都不是中断依据。
