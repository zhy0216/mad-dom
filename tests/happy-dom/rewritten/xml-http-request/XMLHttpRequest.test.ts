...
import type ProgressEvent from '../../shim/src/event/events/ProgressEvent.js';
...
import type ISyncResponse from '../../shim/src/fetch/types/ISyncResponse.js';
...
import { BrowserErrorCaptureEnum } from '../../shim/src/index.js';
...
describe('XMLHttpRequest', () => {
	...
	describe('get status()', () => {
		it('Returns status for synchronous requests.', () => {
			spyOn(SyncFetch.prototype, 'send').mockImplementation(
				() =>
					<ISyncResponse>{
	...
	describe('setRequestHeader()', () => {
		it('Sets a request header on a synchronous request.', () => {
			...
			spyOn(SyncFetch.prototype, 'send').mockImplementation(function () {
				requestArgs = {
					headers: {
		...
		it('Sets a request header on an asynchronous request.', async () => {
			await new Promise((resolve) => {
				...
				spyOn(Fetch.prototype, 'send').mockImplementation(async function () {
					requestArgs = {
						headers: {
							'test-header': this.request.headers.get('test-header')
						}
