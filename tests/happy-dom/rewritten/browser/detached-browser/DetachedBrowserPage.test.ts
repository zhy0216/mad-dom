...
// Upstream path:      packages/happy-dom/test/browser/detached-browser/DetachedBrowserPage.test.ts
...
import DetachedBrowserFrame from '../../../shim/src/browser/detached-browser/DetachedBrowserFrame.js';
import Window from '../../../shim/src/window/Window.js';
import BrowserWindow from '../../../shim/src/window/BrowserWindow.js';
import VirtualConsolePrinter from '../../../shim/src/console/VirtualConsolePrinter.js';
import VirtualConsole from '../../../shim/src/console/VirtualConsole.js';
...
describe('DetachedBrowserPage', () => {
	...
	describe('waitForNavigation()', () => {
		it('Waits page to have been navigated.', async () => {
			...
			spyOn(page.mainFrame, 'waitForNavigation').mockImplementation((): Promise<void> => {
				isCalled = true;
	...
	describe('evaluate()', () => {
		it("Evaluates code in the page's context.", () => {
			...
			spyOn(page.mainFrame, 'evaluate').mockImplementation((code) => {
	...
	describe('goto()', () => {
		it('Goes to a page.', async () => {
			...
			spyOn(page.mainFrame, 'goto').mockImplementation((url, options) => {
				usedURL = url;
	...
	describe('goBack()', () => {
		it('Navigates back in history.', async () => {
			...
			spyOn(page.mainFrame, 'goBack').mockImplementation((options) => {
				usedOptions = <IGoToOptions>options;
	...
	describe('goForward()', () => {
		it('Navigates forward in history.', async () => {
			...
			spyOn(page.mainFrame, 'goForward').mockImplementation((options) => {
				usedOptions = <IGoToOptions>options;
				return Promise.resolve(<Response>{ status: 201 });
			});
	...
	describe('goSteps()', () => {
		it('Navigates a delta in history.', async () => {
			...
			spyOn(page.mainFrame, 'goSteps').mockImplementation((steps, options) => {
	...
	describe('reload()', () => {
		it('Reloads the frame.', async () => {
			...
			spyOn(page.mainFrame, 'reload').mockImplementation((options) => {
				usedOptions = <IGoToOptions>options;
				return Promise.resolve(<Response>{ status: 201 });
			});
