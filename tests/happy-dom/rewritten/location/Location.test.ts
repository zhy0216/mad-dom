...
import type HashChangeEvent from '../../shim/src/event/events/HashChangeEvent.js';
...
describe('Location', () => {
	...
	describe('set host()', () => {
		it('Sets the host of the URL.', () => {
			...
			spyOn(browserFrame, 'goto').mockImplementation(async (url: string) => {
				newURL = url;
				return null;
			});
	...
	describe('set href()', () => {
		it('Calls browserFrame.goto() to navigate to the URL.', () => {
			...
			spyOn(browserFrame, 'goto').mockImplementation(
		...
		it('Handles promise rejections.', async () => {
			...
			spyOn(browserFrame, 'goto').mockImplementation((): Promise<Response | null> => {
