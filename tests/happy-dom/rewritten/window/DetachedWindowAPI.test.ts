...
// Upstream path:      packages/happy-dom/test/window/DetachedWindowAPI.test.ts
...
import { clearAllMocks } from '../../adapter/index.js';
import type DetachedWindowAPI from '../../shim/src/window/DetachedWindowAPI.js';
...
describe('DetachedWindowAPI', () => {
	...
	describe('whenAsyncComplete()', () => {
		it('Calls waitUntilComplete().', async () => {
			let isCalled = false;
			spyOn(<DetachedWindowAPI>window.happyDOM, 'waitUntilComplete').mockImplementation(() => {
				isCalled = true;
	...
	describe('cancelAsync', () => {
		it('Calls abort().', () => {
			let isCalled = false;
			spyOn(<DetachedWindowAPI>window.happyDOM, 'abort').mockImplementation(() => {
				isCalled = true;
				return Promise.resolve();
			});
			window.happyDOM?.abort();
