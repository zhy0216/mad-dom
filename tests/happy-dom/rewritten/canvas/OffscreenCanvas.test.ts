...
// Upstream path:      packages/happy-dom/test/canvas/OffscreenCanvas.test.ts
...
import type OffscreenCanvas from '../../shim/src/canvas/OffscreenCanvas.js';
import ImageBitmap from '../../shim/src/canvas/ImageBitmap.js';
...
describe('OffscreenCanvas', () => {
	...
	describe('transferToImageBitmap()', () => {
		...
		it('Should delegate to adapter.getContext() when adapter is set.', () => {
			...
			const mockContext = { fillRect: mock(), drawImage: mock(), clearRect: mock() };
			const mockContext2 = { fillRect: mock(), drawImage: mock(), clearRect: mock() };
			const mockAdapter: ICanvasAdapter = {
				getContext: mock().mockImplementation(({ canvas }, type) => {
					if (type === '2d') {
