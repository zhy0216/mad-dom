...
describe('SVGFETileElement', () => {
	let window: Window;
	let document: Document;
	let element: SVGFETileElement;

	beforeEach(() => {
		window = new Window();
		document = window.document;
		element = document.createElementNS('http://www.w3.org/2000/svg', 'feTile');
	});

	describe('constructor()', () => {
		it('Should be an instanceof SVGFETileElement', () => {
			expect(element instanceof SVGFETileElement).toBe(true);
		});
