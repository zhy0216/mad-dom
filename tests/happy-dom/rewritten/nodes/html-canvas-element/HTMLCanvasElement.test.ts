...
// Upstream path:      packages/happy-dom/test/nodes/html-canvas-element/HTMLCanvasElement.test.ts
...
// ─────────────────────────────────────────────────────────────────────────────
import HTMLCanvasElement from '../../../shim/src/nodes/html-canvas-element/HTMLCanvasElement.js';
...
import MediaStream from '../../../shim/src/nodes/html-media-element/MediaStream.js';
...
describe('HTMLCanvasElement', () => {
	...
	describe('getContext()', () => {
		...
		it('Should delegate to adapter.getContext() when adapter is set.', () => {
			const mockContext = { fillRect: mock() };
		...
		it('Should pass contextAttributes to adapter.', () => {
			const mockAdapter: ICanvasAdapter = {
				getContext: mock().mockReturnValue(null),
				toDataURL: mock(),
				toBlob: mock()
			};
		...
		it('Should work with different context types.', () => {
			const mockAdapter: ICanvasAdapter = {
				getContext: mock().mockImplementation((_canvas, type) => {
					if (type === '2d') {
						return { type: '2d' };
					}
					if (type === 'webgl') {
		...
		it('Should use different adapters for different windows.', () => {
			const mockAdapter1: ICanvasAdapter = {
				getContext: mock().mockReturnValue({ id: 'window1' }),
				toDataURL: mock(),
				toBlob: mock()
			};
			const mockAdapter2: ICanvasAdapter = {
				getContext: mock().mockReturnValue({ id: 'window2' }),
				toDataURL: mock(),
				toBlob: mock()
			};
		...
		it('Should return null when window has no adapter.', () => {
			const mockAdapter: ICanvasAdapter = {
				getContext: mock().mockReturnValue({ id: 'with-adapter' }),
				toDataURL: mock(),
				toBlob: mock()
			};
	...
	describe('toDataURL()', () => {
		...
		it('Should pass type and quality to adapter.', () => {
			const mockAdapter: ICanvasAdapter = {
				getContext: mock(),
				toDataURL: mock().mockReturnValue('data:image/jpeg;base64,/9j/4AAQ'),
				toBlob: mock()
			};
		...
		it('Should return data URL from OffscreenCanvas when adapter is set and canvas is transferred to OffscreenCanvas.', () => {
			const mockAdapter: ICanvasAdapter = {
				getContext: mock(),
				toDataURL: mock().mockReturnValue('data:image/png;base64,dGVzdA=='),
				toBlob: mock()
			};
	...
	describe('toBlob()', () => {
		...
		it('Should delegate to adapter.toBlob() when adapter is set.', () => {
			...
			const mockAdapter: ICanvasAdapter = {
				getContext: mock(),
				toDataURL: mock(),
				toBlob: mock().mockImplementation((_canvas, callback) => {
					callback(mockBlob);
				})
			};
