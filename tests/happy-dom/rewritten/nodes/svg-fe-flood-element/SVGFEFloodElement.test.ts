...
import SVGFEFloodElement from '../../../src/nodes/svg-fe-flood-element/SVGFEFloodElement.js';
import SVGLength from '../../../src/svg/SVGLength.js';
import SVGElement from '../../../shim/src/nodes/svg-element/SVGElement.js';

describe('SVGFEFloodElement', () => {
	let window: Window;
	let document: Document;
	let element: SVGFEFloodElement;

	beforeEach(() => {
		window = new Window();
		document = window.document;
		element = document.createElementNS('http://www.w3.org/2000/svg', 'feFlood');
	});

	describe('constructor()', () => {
		it('Should be an instanceof SVGFEFloodElement', () => {
			expect(element instanceof SVGFEFloodElement).toBe(true);
		});
