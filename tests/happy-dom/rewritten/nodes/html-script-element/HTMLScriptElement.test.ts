...
import BrowserErrorCaptureEnum from '../../../shim/src/browser/enums/BrowserErrorCaptureEnum.js';
...
describe('HTMLScriptElement', () => {
	...
	describe('set src()', () => {
		it('Does not load external script when JavaScript is disabled.', async () => {
			...
			spyOn(Fetch.prototype, 'send').mockImplementation(
				async () =>
					<Response>{
	...
	describe('set isConnected()', () => {
		...
		it('Does not load script when Javascript is disabled.', () => {
			spyOn(Fetch.prototype, 'send').mockImplementation(async function (this: Fetch) {
				return <Response>{
					text: async () => 'globalThis.test = "test";',
					ok: true
				};
		...
		it('Loads external script synchronously with relative URL.', async () => {
			...
			spyOn(ResourceFetch.prototype, 'fetchSync').mockImplementation(function (
				this: ResourceFetch,
				url: string
			) {
		...
		it('Triggers error event when loading external script synchronously with relative URL.', () => {
			...
			spyOn(ResourceFetch.prototype, 'fetchSync').mockImplementation(() => {
				throw thrownError;
			});
