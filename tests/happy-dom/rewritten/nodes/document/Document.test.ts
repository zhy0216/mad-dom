...
// Upstream path:      packages/happy-dom/test/nodes/document/Document.test.ts
...
import DocumentFragment from '../../../shim/src/nodes/document-fragment/DocumentFragment.js';
import NodeIterator from '../../../shim/src/tree-walker/NodeIterator.js';
import TreeWalker from '../../../shim/src/tree-walker/TreeWalker.js';
import Node from '../../../shim/src/nodes/node/Node.js';
import Document from '../../../shim/src/nodes/document/Document.js';
import type Element from '../../../shim/src/nodes/element/Element.js';
import Event from '../../../shim/src/event/Event.js';
import SVGSVGElement from '../../../shim/src/nodes/svg-svg-element/SVGSVGElement.js';
import SVGElement from '../../../shim/src/nodes/svg-element/SVGElement.js';
import NamespaceURI from '../../../src/config/NamespaceURI.js';
import Attr from '../../../shim/src/nodes/attr/Attr.js';
...
import type HTMLLinkElement from '../../../shim/src/nodes/html-link-element/HTMLLinkElement.js';
import type Response from '../../../shim/src/fetch/Response.js';
import ResourceFetch from '../../../src/fetch/ResourceFetch.js';
import type HTMLScriptElement from '../../../shim/src/nodes/html-script-element/HTMLScriptElement.js';
...
import Selection from '../../../shim/src/selection/Selection.js';
import Range from '../../../shim/src/range/Range.js';
...
import HTMLUnknownElement from '../../../shim/src/nodes/html-unknown-element/HTMLUnknownElement.js';
...
describe('Document', () => {
	...
	describe('get styleSheets()', () => {
		it('Returns all stylesheets loaded to the document.', async () => {
			await new Promise((resolve) => {
				...
				spyOn(Fetch.prototype, 'send').mockImplementation(function () {
	...
	describe('append()', () => {
		it('Inserts a set of Node objects or DOMString objects after the last child of the ParentNode. DOMString objects are inserted as equivalent Text nodes.', () => {
			...
			spyOn(ParentNodeUtility, 'append').mockImplementation((parentNode, ...nodes) => {
	...
	describe('prepend()', () => {
		it('Inserts a set of Node objects or DOMString objects before the first child of the ParentNode. DOMString objects are inserted as equivalent Text nodes.', () => {
			...
			spyOn(ParentNodeUtility, 'prepend').mockImplementation((parentNode, ...nodes) => {
				expect(parentNode === document.body).toBe(true);
				expect(nodes.length).toBe(2);
				expect(nodes[0] === node1).toBe(true);
				expect(nodes[1] === node2).toBe(true);
	...
	describe('replaceChildren()', () => {
		it('Replaces the existing children of a ParentNode with a specified new set of children.', () => {
			...
			spyOn(ParentNodeUtility, 'replaceChildren').mockImplementation((parentNode, ...nodes) => {
				expect(parentNode === document.body).toBe(true);
				expect(nodes.length).toBe(2);
				expect(nodes[0] === node1).toBe(true);
				expect(nodes[1] === node2).toBe(true);
	...
	describe('querySelectorAll()', () => {
		it('Query CSS selector to find matching elements.', () => {
			...
			spyOn(QuerySelector, 'querySelectorAll').mockImplementation((parentNode, selector) => {
	...
	describe('querySelector()', () => {
		it('Query CSS selector to find a matching element.', () => {
			...
			spyOn(QuerySelector, 'querySelector').mockImplementation((parentNode, selector) => {
				expect(parentNode === document).toBe(true);
				expect(selector).toEqual(expectedSelector);
				return element;
			});
	...
	describe('getElementsByClassName()', () => {
		it('Returns an elements by class name.', () => {
			...
			spyOn(ParentNodeUtility, 'getElementsByClassName').mockImplementation(
	...
	describe('getElementsByTagName()', () => {
		it('Returns an elements by tag name.', () => {
			...
			spyOn(ParentNodeUtility, 'getElementsByTagName').mockImplementation(
	...
	describe('getElementsByTagNameNS()', () => {
		it('Returns an elements by tag name and namespace.', () => {
			...
			spyOn(ParentNodeUtility, 'getElementsByTagNameNS').mockImplementation(
	...
	describe('getElementById()', () => {
		it('Returns an element by ID.', () => {
			...
			spyOn(ParentNodeUtility, 'getElementById').mockImplementation(
	...
	describe('addEventListener()', () => {
		...
		it('Triggers "readystatechange" event when all resources have been loaded.', async () => {
			await new Promise((resolve) => {
				...
				spyOn(ResourceFetch.prototype, 'fetch').mockImplementation(async function (
					url: string | URL
				) {
