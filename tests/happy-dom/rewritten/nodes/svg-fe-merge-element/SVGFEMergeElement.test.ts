...
import SVGFEMergeElement from '../../../src/nodes/svg-fe-merge-element/SVGFEMergeElement.js';
import SVGLength from '../../../src/svg/SVGLength.js';
import SVGElement from '../../../shim/src/nodes/svg-element/SVGElement.js';

describe('SVGFEMergeElement', () => {
	let window: Window;
	let document: Document;
	let element: SVGFEMergeElement;

	beforeEach(() => {
		window = new Window();
		document = window.document;
		element = document.createElementNS('http://www.w3.org/2000/svg', 'feMerge');
	});

	describe('constructor()', () => {
		it('Should be an instanceof SVGFEMergeElement', () => {
			expect(element instanceof SVGFEMergeElement).toBe(true);
		});
