...
// Upstream path:      packages/happy-dom/test/module/ModuleURLUtility.test.ts
	...
	describe('getURL()', () => {
		...
		for (const mainField of ['module', 'main']) {
			it(`Resolves "${mainField}" field in Node module URL when "settings.module.resolveNodeModules" is enabled`, () => {
				...
				spyOn(FS, 'readFileSync').mockImplementation((path: any, encoding?: any): any => {
				...
				spyOn(FS, 'existsSync').mockImplementation((path: any): any => {
