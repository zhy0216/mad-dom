...
// Upstream path:      packages/happy-dom/test/fetch/cache/response/ResponseCacheFileSystem.test.ts
...
describe('ResponseCacheFileSystem', () => {
	...
	describe('load()', () => {
		it('Loads cache from file system.', async () => {
			spyOn(Path, 'resolve').mockImplementation((directory: string): string =>
				`${ROOT_DIRECTORY}${directory}`.replace('//', '/')
			);

			spyOn(FS.promises, 'readdir').mockImplementation((directory: any): any => {
			...
			spyOn(FS.promises, 'readFile').mockImplementation((filePath: any, encoding: any): any => {
	...
	describe('save()', () => {
		it('Saves the cache to file system.', async () => {
			...
			spyOn(FS.promises, 'mkdir').mockImplementation((directory: any): any => {
				expect(directory).toBe(ABSOLUTE_DIRECTORY);
				createdDirectories.push(directory);
				return Promise.resolve();
			});

			spyOn(FS.promises, 'writeFile').mockImplementation(
