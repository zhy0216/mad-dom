...
import AbortSignal from '../../shim/src/fetch/AbortSignal.js';
...
import FormData from '../../shim/src/form-data/FormData.js';
	...
	describe('arrayBuffer()', () => {
		...
		it('Supports window.happyDOM?.waitUntilComplete().', async () => {
			await new Promise((resolve) => {
				...
				spyOn(FetchBodyUtility, 'consumeBodyStream').mockImplementation(
					(): Promise<Buffer> =>
	...
	describe('formData()', () => {
		...
		it('Supports window.happyDOM?.waitUntilComplete().', async () => {
			await new Promise((resolve) => {
				...
				spyOn(MultipartFormDataParser, 'streamToFormData').mockImplementation(
