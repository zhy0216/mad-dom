...
// Upstream path:      packages/happy-dom/test/fetch/SyncFetch.test.ts
...
import { PropertySymbol } from '../../shim/src/index.js';
...
describe('SyncFetch', () => {
	...
	describe('send()', () => {
		...
		it('Should return the intercepted response when afterSyncRequest returns a response', () => {
			const url = 'https://localhost:8080/some/path';
			const browser = new Browser({
				settings: {
					fetch: {
		...
		it('Should return the original response when afterSyncRequest does not return a response', () => {
			const url = 'https://localhost:8080/some/path';
			const browser = new Browser({
				settings: {
					fetch: {
						interceptor: {
							afterSyncResponse({ response }) {
								return {
									...response,
									headers: new Headers({ 'Content-Type': 'text/plain' })
								};
		...
		it('Supports POST request with body as FormData.', () => {
			...
			spyOn(Math, 'random').mockImplementation(() => 0.8);
