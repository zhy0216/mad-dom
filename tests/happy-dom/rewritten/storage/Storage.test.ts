...
// Upstream path:      packages/happy-dom/test/storage/Storage.test.ts
...
// ─────────────────────────────────────────────────────────────────────────────
import Storage from '../../shim/src/storage/Storage.js';
...
describe('Storage', () => {
	...
	describe('vi.spyOn()', () => {
		...
		it('Should be able to mock implementation once.', () => {
			spyOn(storage, 'getItem').mockImplementationOnce(() => 'mocked');
			expect(storage.getItem('key1')).toBe('mocked');
			expect(storage.getItem('key1')).toBe(null);

			spyOn(storage, 'setItem').mockImplementationOnce(() => {
		...
		it('Should be able to spy on prototype methods.', () => {
			spyOn(Storage.prototype, 'getItem').mockImplementation(() => 'mocked');
